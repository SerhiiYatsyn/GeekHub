...
export class AppComponent {
  ...
  filterAll(selected) {
    ...
    switch (selected) {
      case 'all': break;
      case 'done':
        this.Tasks.forEach(t => {if(!t.complete) t.hide = true; });
        break;
      case 'undone':
        this.Tasks.forEach(t => {if(t.complete) t.hide = true; });
        break;
      case  'archived':
        this.Tasks.forEach((t => {if(!t.archived) t.hide = true; }));
    }
  }
  ...
  updates(eventParam) {
    switch (eventParam[1]) {
      ...
      case 'editDone':
        ...
        break;
      case 'delete':
        if(eventParam[2] === 'done') this.DoneCount--;
        else if(eventParam[2] === 'undone') this.UnDoneCount--;
        break;
    }
  }
}
