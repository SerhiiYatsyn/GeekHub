document.addEventListener("DOMContentLoaded", function () {
    class Task {
        constructor(task, isDone = false) {
            this._task = task;
            this._isDone = isDone;
        }
        addTask() {
            list.insertAdjacentHTML('beforeend', `<li><input disabled class="edittext" value="${this._task}"><span class="editBtn">EDIT</span><span class="close">X</span></li>`);
        }
    }
    ...
    addBtn.addEventListener('click', function () {
        ...
        if (inputValue !== '') {
            let Taskk = new Task(inputValue, false);
            Taskk.addTask();
            allCount++;
            displayCounters();
        ...
        switch (filter.selectedIndex) {
            case 0:
                for (let i = 0; i < tasks.length; i++) {
                    tasks[i].classList.remove('hiden');
                }
                break;
            case 1:
                for (let i = 0; i < tasks.length; i++) {
                    tasks[i].classList.remove('hiden');
                    if (tasks[i].classList !== 'checked') {
                        tasks[i].classList.add('hiden');
                    }
                }
                break;
            case 2:
